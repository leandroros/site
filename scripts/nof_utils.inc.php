...
/**
 * Returns a random number of chars.
 *
 * @param string 		$chars
...
function nof_captcha_randomChars($chars, $numberOfChars){
			$result = "";
 ...
 * @return string	characters used for the captcha control
 */

function nof_captcha_getCharacters() {
...
/**
 * Validate user input based on the data filled in the form
 * and the existing data from user session.
 *
 * @return boolean
 */

function nof_captcha_validate() {
		$SESSION_KEY = "nof_".GetPostVariable('nof_componentId')."_CaptchSettings";
		$properties = unserialize(GetSessionVariable($SESSION_KEY));

        	if (strtoupper($properties->imageChars)==strtoupper($_POST['captcha'])) return true;
        	else return false; 
}
...
/**
 * Particular case for Testimonials
 *
 * Validate user input based on the data filled in the form
 * and the existing data from user session.
 *
...
function nof_captcha_validate_by_code($componentid) {
	$SESSION_KEY = "nof_".$componentid."_CaptchSettings";
	$properties = unserialize(GetSessionVariable($SESSION_KEY));

	if (strtoupper($properties->codeChars)==strtoupper($_POST['captcha'])) return true;
        else return false; 
}
...
function SetSessionVariable($str, $value) {
	...
	$phpVersionUID=trim(phpversion());
	
	/*
	Due to a major bug in 4.1.2. we use the old way to register session data.
	*/	
	
	if ($phpVersionUID=="4.1.2") {
			global ${$str};
            ...
            if ( isset($this->database[strtolower($key)]) ) {
				...
				if ( count($params) > 0 ) {
					...
					for ( $i=0; $i<count($arrKeys); $i++ ) {
						$value = str_replace($arrKeys[$i], $arrVals[$i], $value);
					}
				}
//                $value = str_replace(array_keys($params),array_values($params),$value);
                $value = preg_replace("/\{[0-9]{1}\}/i","",$value);
...
/*
            if (!array_key_exists(strtolower($key),$this->database)) {
                return $key;
            } else {
                $value = $this->database[strtolower($key)];
                if (substr($value,0,1) == '$') {
...
*/
    ...
    function NOF_fileExists($incfile) {
        global $nof_suiteName;
        if (file_exists($incfile)) {
            return true;
        } else {
            NOF_throwError(103,array("{1}"=>NOF_mapPath($incfile),"{2}"=>$nof_suiteName),false);
            return false;
        }
    }

    function NOF_throwError($code,$params=array(),$stop=true) {
        global $nof_resources;
        if($GLOBALS["nof_debug"] == "true") {
            echo "<p>" . $nof_resources->get("Error.".$code,$params) . "</p>";
        } else {
            echo "<p>" . $nof_resources->get("Error.Text.Generic") . "</p><p>" . $nof_resources->get("Error.Text.Code") . " $code</p>";
        }
        if ($stop) {
            exit();
        }
    ...
    // c:           /   inetpub /   wwwroot /   texte   /   formshandler
    // ..           /   db      /   ..      /   scripts /   .
    // ..(getcwd)   /   db      /
    function NOF_mapPath($path) {
        // if $path is already a path, return it as is
        if ($path[0] == '/' || $path[1] == ':') {
            return addslashes($path);
        }
        // if $path exists, it's quite easy
        if ($mapPath = realpath(($path))) {
            return addslashes($mapPath);
        } else {
            // get current directory as full path
            $dir = getcwd();
            // workaround on path separator
            if (strpos($dir,'\\')>0) {
                $pathSep = '\\';
                $path  = str_replace('/',$pathSep,$path);
            } else {
                $pathSep = '/';
                $path  = str_replace('\\',$pathSep,$path);
            }
            
            $tD = explode($pathSep, $dir);
            $tP = explode($pathSep, $path);
            
            $tDi = count($tD);
            for($ti=0; $ti<count($tP); $ti++){
                if($tP[$ti] == ".."){
                    unset($tD[$tDi]);
                    $tDi--;
                } else {
            ...
            return addslashes(join($pathSep, $tD));
        }
    }

    if (!isset($nof_resources)) {
        $nof_resources = new NOF_resource();
    }

	if ( !function_exists('html_entity_decode') ) {
		function html_entity_decode($str) {
			$htmlEntities = array('<p>', '</p>', "<br>");
			$htmlEntitiesValue = array('\r\n', '\r\n', '\r\n');

			for ( $i=0; $i<count($htmlEntities); $i++ ) {
				$str = str_replace($htmlEntities[$i], $htmlEntitiesValue[$i], $str);
			}
			return $str;
		}
	}
...
class PHPMailer
{
    ...
    //use secure connection if this is true
    var $SMTPSecure  = '';
    ...
    */
    ...
    function Send() {
        ...
        {
            $this->SetError("'missing at least one recipient email address(to||cc||bcc).'");
            return false;
        }
        ...
        switch($this->Mailer)
        {
            ...
            case "smtp":
                ...
                break;
            default:
                $this->SetError("mailer is not supported[sendmail||mail||smtp]");
                $result = false;
                break;
        }
    ...
    function SendmailSend($header, $body) {
        ...
        {
            $this->SetError("Fail to execute: " . $this->Sendmail);
            return false;
        }
    ...
    function SmtpSend($header, $body) {
        ...
        {
            $error = "Address failed, FROM=" . $smtp_from;
            $this->SetError($error);
            $this->smtp->Reset();
            return false;
        }
        ...
        if(count($bad_rcpt) > 0) // Create error message
        {
            ...
            $error = "SMTP Error: recipient(s) failed: " . $error;
            $this->SetError($error);
            $this->smtp->Reset();
            return false;
        }
